...
The first section will treat supervised text classification, which will act as a foundation for the active learning that will come after.
...
This chapter will then end by going through the different evaluation metrics that will be used to assess the methods used.
...
In their paper, Yang et al\@.~\cite{yang2009effective} consider the process of finding the greatest reduction in two steps: finding a good estimate for the conditional probability $p(y|x)$, and finding a way to assess the loss reduction of a multi-label classifier.
...
The only thing that remains is to combine the estimation of $p(x|y)$ with the estimate for loss reduction.
