...
Text classification, primarily using support vector machines and logistic regression, will be covered in the section after that.
Following this, there will be a section containing an overview of the field of active learning, as well as a comparison of some different active learning techniques 
...
\begin{enumerate}
    ...
    \item A machine learning model is trained on the labeled pool, this model is often used by the querying strategy in order to select samples to label in step~\ref{enum:label}
\end{enumerate}
...
After which some concrete methods for selecting the samples are described and compared.
...
The stream-based approach draws one sample at a time from an input source and make the decision whether or not to query a label for it.
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
, where $\mathcal{W}$ is our parameter space and $w \in \mathcal{W}$.
...
What uncertainty sampling is doing is predicting the values for the unlabeled points, and then choosing the one that it is most uncertain about.
...
Furthermore, the probabilities for these $m$ labels are probably significantly higher than the probabilities for the labels that follows.
