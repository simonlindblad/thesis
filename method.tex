...
The task was to evaluate how well unsupervised techniques could be used to filter out invalid reports.
...
In order to use the LDA model the number of topics as to be selected, as described in Section~\ref{sec:topic-modeling}.
Selecting the best model was done by evaluating different number of topics $k_{LDA}$.
The different values that were evaluated was 25, 50, 75, 100, 150, 200.
...
In order to evaluate how well the models performed on the clinical data, a set of reports had to be marked as valid/invalid.
This was done by creating a script that presented a report to the user, and allowed it to be marked as valid or invalid.
5358 reports were labeled by the author, out of wich 623 were marked as invalid. 
...
First, the different toipcs were manually identified using the aforementioned visualization tools.
...
He et al\@. discusses the usage of ROC curves and measures such as g-means to compare multi-class imbalanced data~\ref{he2009learning}.
